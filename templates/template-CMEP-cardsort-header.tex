...
\newcommand*{\headertitlefont}{\small\color{headertextcol}\bfseries}
\newcommand*{\footerfont}{\scriptsize}
\newcommand*{\stationbuttonfont}{\color{white}\bfseries}
\newcommand*{\pvidbuttonfont}{\color{black}\itshape}
\newcommand*{\headspace}{\hspace{1ex}}
\newcommand*{\logoheight}{\headheight-2pt}
\newcommand*{\bbox}[2][white]{\colorbox{#1}{$\displaystyle #2$}}

%% Define the headers
% Clear all headers
\fancyhead{}

% On all pages, the left of the header contains the logo vertically centred
\fancyhead[L]{%
  $\vcenter{\hbox{\includegraphics[height=\logoheight]{cmep-logo3}}}$
  \headerlogofont Cambridge Mathematics
}

% Page-specific layouts define the centre and right headers

%% Define the footers
% Clear all footers
\fancyfoot{}
% Page-specific layouts should define the left footer, containing last upated from the meta.
% Since many sheets are single page, it might not make senst to put the page number
% and also, our footers are quite long and often run into the centre
\fancyfoot[R]{%
